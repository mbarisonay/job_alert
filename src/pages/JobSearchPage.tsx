    ...
    <AppLayout>
      <section className="flex flex-col gap-4">
        <div className="flex items-center justify-between gap-3">
          ...
          <Badge className="hidden text-[11px] text-slate-300 md:inline-flex">
        ...
        <div className="grid gap-4 md:grid-cols-[minmax(0,40%),minmax(0,60%)] md:h-[calc(100vh-10rem)]">
          {/* Sol: Liste */}
          <div className="flex flex-col gap-2 overflow-hidden rounded-lg border border-slate-800 bg-slate-950/60">
            <div className="flex items-center justify-between border-b border-slate-800 px-3 py-2 text-xs text-slate-400 md:hidden">
        ...
        {mobileDetailId && (
          <MobileJobSheet
            job={filteredJobs.find((j) => j.id === mobileDetailId) ?? selectedJob}
  ...
  return (
    <div className="flex gap-2">
      <input
        ...
        className="h-9 w-full rounded-md border border-slate-800 bg-slate-950/80 px-3 text-xs text-slate-100 outline-none ring-0 placeholder:text-slate-500 focus:border-slate-500 focus:ring-2 focus:ring-slate-700"
        value={q}
...
function JobDetail({ job }: JobDetailProps) {
  return (
    <div className="sticky top-[4.5rem] flex h-[calc(100vh-10rem)] flex-col rounded-lg border border-slate-800 bg-slate-950/80 p-4">
      ...
      <div className="mt-4 flex-1 space-y-3 overflow-y-auto text-xs text-slate-300">
        <p>
          Bu alan şimdilik sahte içerik ile doldurulmuş durumda. Gerçek senaryoda
          ilan açıklaması, aranan nitelikler, şirket bilgisi ve faydalar burada
          gösterilecek.
        </p>
        <p>
...
function MobileJobSheet({ job, onClose }: MobileJobSheetProps) {
  return (
    <div className="fixed inset-0 z-40 bg-black/60 backdrop-blur-sm md:hidden">
      <div className="absolute inset-x-0 bottom-0 max-h-[85vh] rounded-t-2xl border border-slate-800 bg-slate-950">
        <div className="flex items-center justify-center pt-2">
          <div className="h-1 w-10 rounded-full bg-slate-700" />
        </div>
        <div className="flex items-center justify-between px-4 py-2">
          <p className="text-xs font-medium text-slate-200">İlan Detayı</p>
          <button
            type="button"
            onClick={onClose}
