...
export function JobCard({ job, isActive, onClick }: JobCardProps) {
  ...
  return (
    <button
      ...
      <Card
        className={`flex gap-3 border-slate-300 dark:border-slate-800/70 bg-white dark:bg-slate-900/70 transition hover:border-slate-200 dark:hover:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-900 ${
          isActive
            ? "border-slate-400 dark:border-slate-500 ring-1 ring-slate-400/40"
            : ""
        }`}>
        <div className="flex h-full items-start justify-center pl-4 pt-4">
          <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-slate-200 dark:bg-slate-800 text-slate-900 dark:text-slate-50">
        ...
        <div className="flex flex-1 flex-col">
          <CardHeader className="pb-2 pr-4">
            <div className="flex items-start justify-between gap-2">
              ...
              <Badge variant={scoreVariant}>{job.aiScore}% Uyumlu</Badge>
            </div>
          </CardHeader>
          ...
          <CardContent className="flex flex-wrap items-center gap-3 pb-3 pr-4">
            ...
            {job.salaryRange && (
              <div className="inline-flex items-center gap-1.5 text-xs text-slate-700 dark:text-slate-300">
                <PiggyBank className="h-3.5 w-3.5 text-slate-500 dark:text-slate-400" />
